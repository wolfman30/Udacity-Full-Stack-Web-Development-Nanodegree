...
query1 = """SELECT FORMAT('---->%s: %s views', title, count(*)) 
            FROM articles, log 
...
query2 = """SELECT FORMAT('---->%s: %s views', authors.name, sum(num_of_views))
            FROM authors
            JOIN articles
...
query3 = """
        SELECT FORMAT('---->%s: %s%% errors', day1, ROUND(num_errs/cast(total_requests as decimal)*100, 2)) 
        FROM 
            (SELECT TO_CHAR(time, 'Mon DD, YYYY') AS day1, count(status) num_errs 
            FROM log                                                        
            WHERE status = '404 NOT FOUND'                                   
            GROUP BY 1) AS errors_per_day, 
            (SELECT TO_CHAR(time, 'Mon DD, YYYY') AS day2, count(*) total_requests 
            FROM log 
